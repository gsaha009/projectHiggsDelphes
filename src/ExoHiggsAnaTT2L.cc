...
// Constructor
// -----------
ExoHiggsAnaTT2L::ExoHiggsAnaTT2L()
{
  fileList_.clear();
  hmap_.clear();
}
...
// -------------------------------------------------------
//bool ExoHiggsAnaTT2L::beginJob(const string& inputFile, string& outputFile)  
//bool ExoHiggsAnaTT2L::beginJob(const std::string& jobFile, std::string& outputFile)  
//bool ExoHiggsAnaTT2L::beginJob(const std::string& jobFile)  
bool ExoHiggsAnaTT2L::beginJob()  
{ 
...
// Book histograms
// ---------------
void ExoHiggsAnaTT2L::bookHistograms()
{
  histf->cd();
  ...
  new TH1D("Electron4Pt", "pT of 4th Electron",400, 0.0, 800.0);
  ...
  if (readGenInfo()){
    new TH1D("GenCutFlow", "", 4, -0.5, 3.5);
    new TH1D("GenLepFlv", "flavors of gen leptons", 3, -0.5, 2.5);
    new TH1D("ngenLeptons", "", 10, -0.5, 9.5);  
    new TH1D("GenLep1pt", "", 200, 0.0, 400.0);
    new TH1D("GenLep2pt", "", 200, 0.0, 400.0);
    new TH1D("GenLep3pt", "", 200, 0.0, 400.0);
    new TH1D("nGenLeptons_", "", 10, -0.5, 9.5);
    new TH1D("nDetLeptons_", "", 10, -0.5, 9.5);
    new TH1D("minGenMuDR", "", 100, -0.5, 9.5);
    new TH1D("minGenEleDR", "", 100, -0.5, 9.5);
    new TH1D("nGenMatchedMuons", "", 10, -0.5, 9.5);  
    new TH1D("nGenMatchedElectrons", "", 10, -0.5, 9.5);
    new TH1D("nGenMatchedLeptons", "", 10, -0.5, 9.5);  
  }
  histf->cd();
  histf->ls();
}
...
void ExoHiggsAnaTT2L::clearLists() {
  MuonColl.clear();
  ElectronColl.clear();
  JetColl.clear();
}
...
void ExoHiggsAnaTT2L::eventLoop(ExRootTreeReader *treeReader)
{
  ...
  //  for (size_t iEntry = 0; iEntry < 10000; ++iEntry) {
    bool verbose {false};
    ...
    if (ElectronColl.size() > 3) AnaUtil::fillHist1D("Electron4Pt", ElectronColl[3].PT, evWt);
    ...
    if (readGenInfo()) {
      AnaUtil::fillHist1D ("GenCutFlow", 0, evWt);
      for (auto *_gp: *BrGenP){ 
	...
	if (absMomId != 24) continue;
	GenLepton.push_back(gp);
      }
      std::sort(std::begin(GenLepton), std::end(GenLepton), PtComparator<GenParticle>());
      AnaUtil::fillHist1D ("ngenLeptons", GenLepton.size(), evWt);
      if (GenLepton.size() > 0) AnaUtil::fillHist1D ("GenLep1pt", GenLepton[0].PT, evWt);
      if (GenLepton.size() > 1) AnaUtil::fillHist1D ("GenLep2pt", GenLepton[1].PT, evWt);
      if (GenLepton.size() > 2) AnaUtil::fillHist1D ("GenLep3pt", GenLepton[2].PT, evWt);

      if (GenLepton.size() < 2) continue;
      AnaUtil::fillHist1D ("GenCutFlow", 1, evWt);
      double genLepMinPt  = 999.0;
      double genLepMaxEta = 0.00001;
      bool hasTau {false};
      for (auto &gl: GenLepton){
	if (std::fabs(gl.PID) == 11) AnaUtil::fillHist1D("GenLepFlv", 0, evWt);
	if (std::fabs(gl.PID) == 13) AnaUtil::fillHist1D("GenLepFlv", 1, evWt);
	if (std::fabs(gl.PID) == 15) {
	  AnaUtil::fillHist1D("GenLepFlv", 2, evWt);
	  hasTau = true;
	}
	double genLepPt  = gl.PT;
	double genLepEta = std::fabs(gl.Eta);
	if (genLepPt < genLepMinPt) genLepMinPt = genLepPt;
	if (genLepEta > genLepMaxEta) genLepMaxEta = genLepEta; 
      }
      if (genLepMinPt < 7 || genLepMaxEta > 2.5) continue;
      AnaUtil::fillHist1D ("GenCutFlow", 2, evWt);
      if (hasTau) continue;
      AnaUtil::fillHist1D ("GenCutFlow", 3, evWt);
      
      AnaUtil::fillHist1D ("nGenLeptons_", GenLepton.size(), evWt);
      AnaUtil::fillHist1D ("nDetLeptons_", (BrMuon->GetEntriesFast() + BrElec->GetEntriesFast()), evWt);

      std::vector<Muon>GenMatchedMuons;
      std::vector<Electron>GenMatchedElectrons;
      for (TObject *_mu: *BrMuon){ 
	//	cout<<"counting muons"<<endl;
	const Muon& mu = dynamic_cast<const Muon&> (*_mu);
	TLorentzVector mup4 = mu.P4();
	double minGenMuDR = 999.9;
	for (auto &gl: GenLepton) {
	  TLorentzVector glp4 = gl.P4();
	  double genMuDR = mup4.DeltaR(glp4);
	  //	  cout<<"dr: "<<genMuDR<<endl;
	  if (genMuDR < minGenMuDR) minGenMuDR = genMuDR;
	}
	AnaUtil::fillHist1D("minGenMuDR", minGenMuDR, evWt);
	if (minGenMuDR < 0.1) GenMatchedMuons.push_back(mu);
      }
      AnaUtil::fillHist1D("nGenMatchedMuons", GenMatchedMuons.size(), evWt);


      for (TObject *_el: *BrElec){ 
	const Electron& el = dynamic_cast<const Electron&> (*_el);
	TLorentzVector elp4 = el.P4();
	double minGenEleDR = 999.9;
	for (auto &gl: GenLepton) {
	  TLorentzVector glp4 = gl.P4();
	  double genEleDR = elp4.DeltaR(glp4);
	  if (genEleDR < minGenEleDR) minGenEleDR = genEleDR;
	}
	AnaUtil::fillHist1D("minGenEleDR", minGenEleDR, evWt);
	if (minGenEleDR < 0.1) GenMatchedElectrons.push_back(el);
      }
      AnaUtil::fillHist1D("nGenMatchedElectrons", GenMatchedElectrons.size(), evWt);

      AnaUtil::fillHist1D ("nGenMatchedLeptons", (GenMatchedMuons.size() + GenMatchedElectrons.size()), evWt);
    }// if (readGenInfo()) scope ends
...
void ExoHiggsAnaTT2L::endJob() {
  ...
  vector<string> genEvLabels {
    "0) Events processed              : ",
      "1) nLeptons >= 2               : ",
      "2) minPt > 7 & maxEta < 2.5    : ",
      "3) has no Tau Cand in GenLevel : "
      };
  
  AnaUtil::showEfficiency("evtCutFlow", evLabels, "Event Selection");  
  if (readGenInfo()) {
    std::cout<<"Gen Level Efficiency ==================>"<<std::endl;
    AnaUtil::showEfficiency("GenCutFlow", genEvLabels, "Gen Event Selection");
  }
  double lumiFac = 1.0;
...
void ExoHiggsAnaTT2L::closeHistFiles(){
  histf->cd();
  histf->Write();
  histf->Close();
}
void ExoHiggsAnaTT2L::closeFiles(){
  closeHistFiles();
  delete treeReader;
  delete chain;
}
bool ExoHiggsAnaTT2L::readJob(const string& jobFile, int& nFiles)
{
...
void ExoHiggsAnaTT2L::printJob(ostream& os) const{
  os << endl;
...
double ExoHiggsAnaTT2L::lumiWt(double evtWeightSum, int maxEvents) const{
...
int ExoHiggsAnaTT2L::getMotherId(const GenParticle& gp, TClonesArray *gen, int& mmid) const {
  int pdgid = gp.PID;
  int indx = gp.M1;
  if (indx < 0) return -1;
...
bool ExoHiggsAnaTT2L::jetLeptonCleaning(const Jet& jet, std::vector <Muon> muList_, std::vector <Electron> eleList_, double dR) {
...
void ExoHiggsAnaTT2L::dumpGenInfo(TClonesArray *gen, ostream& os) {

...
// Constructor
// -----------
ExoHiggsAna_ZZ4L::ExoHiggsAna_ZZ4L()
{}
...
// -------------------------------------------------------
//bool ExoHiggsAna_ZZ4L::beginJob(const char* inputFile, char* outputFile)  
bool ExoHiggsAna_ZZ4L::beginJob(const string& inputFile, string& outputFile)  
{ 
...
// Book histograms
// ---------------
void ExoHiggsAna_ZZ4L::bookHistograms()
{
  histf->cd();
  ...
  new TH1D("Muon5Pt", "pT of 5th Muon",400, 0.0, 800.0);
  ...
  new TH1D("Electron5Pt", "pT of 5th Electron",400, 0.0, 800.0);
  ...
  new TH1D("Jet7Pt", "pT of 7th Jet",400, 0.0, 800.0);
...
void ExoHiggsAna_ZZ4L::clearLists() {
  MuonColl.clear();
  ElectronColl.clear();
  JetColl.clear();
}
...
void ExoHiggsAna_ZZ4L::eventLoop(ExRootTreeReader *treeReader)
{
  ...
  for (size_t iEntry = 0; iEntry < 50000; ++iEntry) {
    
    bool verbose {false};
    if ((iEntry/10000) > 0 && iEntry%1000 == 0) verbose = true;
    ...
    if (MuonColl.size() > 4) AnaUtil::fillHist1D("Muon5Pt", MuonColl[4].PT, evWt);
    ...
    if (ElectronColl.size() > 4) AnaUtil::fillHist1D("Electron5Pt", ElectronColl[4].PT, evWt);
    ...
    if (JetColl.size() > 6) AnaUtil::fillHist1D("Jet7Pt", JetColl[6].PT, evWt);
    
    ////////////////////////// G E N   A N A L Y S I S //////////////////////////
    ...
    for (auto *_gp: *BrGenP){ 
      ...
      int mmid = -1;                                
      if (!((abs_pdgid == 11 || abs_pdgid == 13 || abs_pdgid == 15) && status == 1)) continue;
    ...
    if (ZCand.mass < 60 || ZCand.mass > 120) continue;
    ...
    auto result = leptonPairSelector(MuonColl, ElectronColl, ZCand, leptonPairCandList, 170.0, evWt, 0.1);
    histf->cd();
    histf->cd("Analysis");
...
void ExoHiggsAna_ZZ4L::closeHistFiles(){
  histf->cd();
  histf->Write();
  histf->Close();
}
void ExoHiggsAna_ZZ4L::closeFiles(){
  closeHistFiles();
  delete treeReader;
  delete chain;
}
...
#if 0
void ExoHiggsAna_ZZ4L::printJob(ostream& os) const
{
  AnaBase::printJob(os);
  os << endl;
...
#endif
int ExoHiggsAna_ZZ4L::getMotherId(const GenParticle& gp, TClonesArray *gen, int& mmid) const {
  int pdgid = gp.PID;
  int indx = gp.M1;    
  if (indx < 0) return -1;

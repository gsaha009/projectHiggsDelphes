...
// -------------------------------------------------------
//bool ExoH2Z::beginJob(const string& inputFile, string& outputFile)  
//bool ExoH2Z::beginJob(const std::string& jobFile, std::string& outputFile)  
//bool ExoH2Z::beginJob(const std::string& jobFile)  
bool ExoH2Z::beginJob()  
{ 
...
void ExoH2Z::eventLoop(ExRootTreeReader *treeReader)
{
...
void ExoH2Z::closeHistFiles(){
  histf->cd();
  histf->Write();
  histf->Close();
}
void ExoH2Z::closeFiles(){
  closeHistFiles();
  if (skimObj_ != nullptr) skimObj_->close();
  delete treeReader;
  delete chain;
}
bool ExoH2Z::readJob(const string& jobFile, int& nFiles)
{
...
void ExoH2Z::printJob(ostream& os) const{
  os << endl;
...
double ExoH2Z::lumiWt(double evtWeightSum, int maxEvents) const{
...
int ExoH2Z::getMotherId(const GenParticle& gp, TClonesArray *gen, int& mmid) const {
  int pdgid = gp.PID;
  int indx = gp.M1;
  if (indx < 0) return -1;
...
bool ExoH2Z::jetLeptonCleaning(const Jet& jet, std::vector <Muon> muList_, std::vector <Electron> eleList_, double dR) {
...
void ExoH2Z::dumpGenInfo(TClonesArray *gen, ostream& os) {

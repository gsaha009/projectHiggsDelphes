...
void ExoXZ::eventLoop(ExRootTreeReader *treeReader)
{
...
void ExoXZ::closeHistFiles(){
  histf->cd();
  histf->Write();
  histf->Close();
}
void ExoXZ::closeFiles(){
  closeHistFiles();
  if (skimObj_ != nullptr) skimObj_->close();
  delete treeReader;
  delete chain;
}
bool ExoXZ::readJob(const string& jobFile, int& nFiles)
{
...
void ExoXZ::printJob(ostream& os) const{
  os << endl;
...
double ExoXZ::lumiWt(double evtWeightSum, int maxEvents) const{
...
int ExoXZ::getMotherId(const GenParticle& gp, TClonesArray *gen, int& mmid) const {
  int pdgid = gp.PID;
  int indx = gp.M1;
  if (indx < 0) return -1;
...
bool ExoXZ::jetLeptonCleaning(const Jet& jet, std::vector <Muon> muList_, std::vector <Electron> eleList_, double dR) {
...
void ExoXZ::dumpGenInfo(TClonesArray *gen, ostream& os) {
